 ...
 */
public class LotSystem extends EntitySystem implements EntityListener  {
    ...
    ComponentMapper<LotCharacterComponent> cm = ComponentMapper.getFor(LotCharacterComponent.class);

    public LotSystem(BulletSystem bulletSystem) {
    ...
    public void addedToEngine(Engine e) {
        entities = e.getEntitiesFor(Family.all(LotComponent.class, LotCharacterComponent.class).get());
...
//        }
        for (Entity e : entities) {
            ...
            lotPosition = mod.instance.transform.getTranslation(lotPosition);
            float dX = playerPosition.x - lotPosition.x;
            float dZ = playerPosition.z - lotPosition.z;

            float dXS = 0 - lotPosition.x;
            float dZS = 0 - lotPosition.z;
            ...
            */
            ...
            if((playerPosition.x - lotPosition.x) <= 10 && (playerPosition.x - lotPosition.x) >= -10
                    && (playerPosition.z - lotPosition.z) <= 10 && (playerPosition.z - lotPosition.z) >= -10){
...
//            mod.instance.transform.setToTranslation(translation.x - translation.x, translation.y, translation.z- translation.z);
            ...
            if(Gdx.input.isKeyJustPressed(Input.Keys.Z)){
                System.out.println("this is x: " + (playerPosition.x - lotPosition.x));
                System.out.println("this is z: " + (playerPosition.z - lotPosition.z));
            }
